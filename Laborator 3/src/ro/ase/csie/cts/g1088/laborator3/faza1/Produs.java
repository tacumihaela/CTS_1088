	...
	public float getPretFinal(TipProdus tipProdus, float pretInitial, int vechimeClientInAni)
	  {
	    ...
	    {
	      pretFinal = (pretFinal - (0.1f * pretInitial)) - discountFidelitate * (pretInitial - (0.1f * pretInitial));
	    }
	    else if (tipProdus == TipProdus.STOC_LIMITAT)
	    {
	      pretFinal = (pretFinal - (0.25f * pretInitial)) - discountFidelitate * (pretInitial - (0.25f * pretInitial));
	    }
	    else if (tipProdus == TipProdus.VECHI)
	    {
	      pretFinal = (pretFinal - (0.35f * pretInitial)) - discountFidelitate * (pretInitial - (0.35f * pretInitial));
	    }
	    return pretFinal;
	  }
}